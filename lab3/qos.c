 ...
 * srTCM
 */
int qos_param_init(uint64_t CIR, uint64_t CBS, uint64_t EBS, int index)
{
    srtcmParams[index].cir = CIR;
    srtcmParams[index].cbs = CBS;
    srtcmParams[index].ebs = EBS;
    int ret;
    ret = rte_meter_srtcm_config(&srtcmFlows[index], &srtcmParams[index]);
    if (ret != 0)
    {
        rte_panic("qos_param_init error!\n");
        return ret;
    }
    return 0;
}
...
int ret_params_init(const uint16_t wq_log2, const uint16_t min_th, const uint16_t max_th, const uint16_t maxp_inv, enum qos_color color)
{
    if (rte_red_config_init(&redParams[color], wq_log2, min_th, max_th, maxp_inv))
    {
        rte_panic("ret_params_init error!\n");
        return -1;
    }

    for (int i = 0; i < APP_FLOWS_MAX; i++)
    {
        if (rte_red_rt_data_init(&app_red[i][color]))
        {
            rte_panic("ret_params_init error2!\n ");
            return -1;
        }
    }
}
